...
function Customize2(){
    ...
    return(
        ...
        flex justify-center items-center flex-col p-[20px] relative">
            ...
            {assistantName && <button className="min-w-[300px] h-[60px] mt-[30px] 
            text-black font-semibold cursor-pointer bg-white rounded-full text-[19px]" disabled={loading}
            onClick={()=>{
                //navigate("/customize2")
                handleUpdateAssistant()}}>{!loading?"Finally Create Your Assistant":"Loading..."}</button>}
        </div>
    )
}
